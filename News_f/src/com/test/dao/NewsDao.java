...
public class NewsDao implements NewsDaoInter{
    ...
    public List<News> getNewsByType(String type) {
        ...
        String sql = "select type,title,author,createdate,count,id from news where 1=1 ";
        if (type != null) {
        ...
        try {
            ...
            while (resultSet.next()) {
                ...
                news.setCount(resultSet.getInt(5));
                news.setId(resultSet.getInt(6));
                //将学生对象存放到list中
                list.add(news);
            }
        } catch (SQLException e) {
    ...
    public News getNewsById(int id) {
        News news = null;
        String sql = "select id,type,title,author,createdate,modifydate,content,count from news where id=?";
        connection = dataBase.openConn();
        try {
            preparedStatement = connection.prepareStatement(sql);
            //设置参数
            ...
            if (resultSet.next()) {
                ...
                news.setCount(resultSet.getInt(8));
            }
        } catch (SQLException e) {
            e.printStackTrace();
        } finally {
    ...
    public boolean editNews(News news) {
        ...
        String sql = "update news set type=?,title=?,author=?,modifydate=?,content=?,count=? where id=?";
        connection = Database.openConn();
        try {
            preparedStatement = connection.prepareStatement(sql);
            preparedStatement.setString(1,news.getType());
    ...
    public List<News> SearchNews(String keyword) {
        List<News> list = new ArrayList<News>();
        String sql = "SELECT* FROM news where(content like ? or title LIKE ? or type like ? or author like ?)";
        connection = Database.openConn();
        try {
            preparedStatement = connection.prepareStatement(sql);
